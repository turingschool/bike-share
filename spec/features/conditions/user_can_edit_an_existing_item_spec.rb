...
RSpec.describe "When a user edits a condition" do
  it "can edit all feilds" do
    ...
    expect(page).to have_content("Date: 07/20/2013")
    expect(page).to have_content("Highest Temp: 83.0°F")
    expect(page).to have_content("Mean Temp: 72.0°F ")
    expect(page).to have_content("Lowest Temp: 62.0°F")
