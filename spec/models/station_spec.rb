...
RSpec.describe Station do
  describe 'validations' do
    ...
    it 'is invalid if its missing a name' do
      station = Station.create( dock_count: 12, city: 'Denver', installation_date: Date.parse('8/6/2013'))

      expect(station).to_not be_valid
    end

    it 'is invalid if its missing a dock count' do
      station = Station.create(name: 'Turing Station', city: 'Denver', installation_date: Date.parse('8/6/2013'))

      expect(station).to_not be_valid
    end

    it 'is invalid if its missing a city' do
      station = Station.create(name: 'Turing Station', dock_count: 12, installation_date: Date.parse('8/6/2013'))
